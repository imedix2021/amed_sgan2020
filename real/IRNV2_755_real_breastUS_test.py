...
# Data aquisition
# ZIP read - Rewrite the path according to the location of the 'real' folder.
z = zipfile.ZipFile('/real/hikakudata.zip')
...
np.savetxt('correctIncResNetV2_775_real.csv',true_classes,delimiter=',')

# Test data n predictive labels (n = 300)
pred_classes = np.argmax(model.predict(X_test[0:300]), axis = 1)
print('prediction:', pred_classes)
np.savetxt('predictionsIncResNetV2_775_real.csv',pred_classes,delimiter=',')

# Confusion Matrix
print('2x2_IncRenNetV2_775')
# Benign Prediction
#                     Predicted
#                 Malig(0)  Benign(1)
...
plt.grid()
plt.savefig('/home/imedix/AMED_PGGAN_CLASS/Real/Real_BreastUS_IRNV2_775_Test_roc_curve.png')
